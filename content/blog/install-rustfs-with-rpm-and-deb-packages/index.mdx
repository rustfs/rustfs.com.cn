...
RustFS 从 `1.0.0-rc.1` 开始正式提供 `rpm/deb` 安装包，用户可以通过 `dpkg` 命令快速完成 RustFS 的安装。本文分享如何在 Ubuntu 24.04 上通过 rpm/deb 安装包来安装一个单机多盘实例。
...
rc 是 RustFS Client CLI，可以用来对 RustFS 实例进行操作，诸如对象/存储桶的操作、存储池的操作、集群状态查看等。
