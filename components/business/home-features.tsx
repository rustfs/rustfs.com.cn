      ...
      <div className='mx-auto max-w-7xl px-4 sm:px-6 lg:px-8'>
        ...
        <div className="mx-auto mb-10 text-center lg:mb-14">
          <h2 className="text-3xl font-bold md:text-4xl tracking-wide md:leading-tight text-foreground">
            超强性能的 <span className="text-brand">企业级分布式文件系统</span>
          </h2>
                  ...
                  <ul className="text-muted-foreground grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                    {feature.features.map((item, itemIndex) => (
                      <li className="flex gap-3 items-start" key={itemIndex}>
                        ...
                        <span>{typeof item === 'string' ? item : item['zh']}</span>
                      </li>
                    ))}
                  </ul>
                  ...
                  <div className="border-t border-border pt-6">
                    <div className="flex flex-col md:flex-row items-center gap-4">
                      ...
                      <p className="text-muted-foreground flex-1 text-lg italic">
                        &ldquo;{typeof feature.review.review === 'string' ? feature.review.review : feature.review.review['zh']}&rdquo;
                      </p>
                      <div className="flex items-center gap-4 self-end">
                        <div className="text-right">
                          <p className="font-semibold text-foreground">
                            {typeof feature.review.name === 'string' ? feature.review.name : feature.review.name['zh']}
                          </p>
                          <p className="text-sm text-muted-foreground">
                            {typeof feature.review.position === 'string' ? feature.review.position : feature.review.position['zh']}
                          </p>
                        </div>
                        <img
